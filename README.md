...
提供精准的文本与 JSON 差异比对能力：

<div align="center">
<img src="./docs/compare.png" width="600" alt="Generator Tool Preview" />
</div>

- **直观比对**: 双侧视图同步滚动，精确到字符级别的差异高亮。
- **极速响应**: 基于原生视图引擎构建，支持长文本的瞬间解析与流畅预览。
- **快速对比**: 深度集成系统剪贴板，复制内容自动加入比对。
- **智能过滤**: 一键忽略空格、换行及大小写差异，专注核心逻辑。
- **快捷导航**: 自动统计差异总数，支持在各个差异区块间快速跳转定位。
- **拖拽支持**: 直接将文本文件拖入编辑区，即刻读取并开启比对。
- **JSON 增强**: 内置 JSON 识别与格式化，支持按键名排序后精准比对。
...
<div align="center">
<img src="./docs/automation.png" width="600" alt="Generator Tool Preview" />
</div>
...
由于 macOS 的安全机制，打开非 App Store 下载的应用可能会触发此提示。可以按照下面方法快速修复：

打开终端，执行以下命令：

```
sudo xattr -rd com.apple.quarantine "/Applications/Yota.app"
```
